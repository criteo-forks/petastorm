...
##################################################################################
### namedtuple2 -- Alternative implementation that can cope wtith > 255 arguments
##################################################################################
...
class {typename}(tuple):
    ...
    def __new__(_cls, *args, **kwargs):
        ...
        for arg in args:
            targs.append(arg)
        targs.extend([None] * (len(_cls._fields) - len(targs)))
        for index, name in enumerate(_cls._fields):
            if name in kwargs:
                targs[index] = kwargs[name]
        return _tuple.__new__(_cls, tuple(targs))
...
_field_template = '''\
    {name} = _property(_itemgetter({index}), doc='Alias for field number {index}')
'''


def namedtuple2(typename, field_names, *, verbose=False, rename=False, module=None):
    ...
    namespace = dict(__name__='namedtuple2_%s' % typename)
